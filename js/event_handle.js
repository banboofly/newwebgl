...
function onDocumentMouseDown( event ) {
    ...
    lastX = event.clientX;
    lastY = event.clientY;
    scrollStart(event.clientX, event.clientY);
}
...
function onDocumentMouseMove( event ) {
    if ( isUserInteracting === true ) {
        ...
        scrollMove(event.clientX, event.clientY);
        lastX = event.clientX;
        lastY = event.clientY;
    }
}
...
function callBack(dx, dy){
    var focus = (canvas.height * 0.5) / Math.tan(fovy*0.5);
    var deltaYaw = Math.atan2(dx, focus);
    var deltaPitch = Math.atan2(dy, focus);
...
function onDocumentMouseUp( event ) {
    scrollEnd(event.clientX,event.clientY, callBack);
	...
	switch(e.type){
		case 'touchstart':
			...
			if (e.touches.length > 1) {
                ...
                scrollStart(e.touches[0].clientX, e.touches[0].clientY);
		...
		case 'touchmove':
			if (e.touches.length == 2) {
				...
				if ( isUserInteracting == true ) {
                    ...
                    scrollMove(e.touches[0].clientX, e.touches[0].clientY);
					lastX = e.touches[0].clientX;
					lastY = e.touches[0].clientY;
				}
			}
		...
		case 'touchend':
			if(isCanScale){
                ...
                scrollEnd(lastX, lastY, callBack);
            }
			isUserInteracting = false;
